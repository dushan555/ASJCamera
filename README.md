...
`Invert Forward Motion` 单独反转整只手的前后估计位移，不改变左右、上下或手背姿态。当前场景已开启，前后方向相对上一版反向；关闭可恢复上一版方向。运行中切换会先释放抓取，避免物体跳动。骨架预览为正交投影，Z 变化不会自动产生近大远小的透视效果，应结合关节 Z 或抓取物体的位置判断方向。
